...
multmerge = function(filenames){ #Pfad im ordener wo die csv sind muss angegeben werden.
  ...
  Reduce(function(x,y) {merge(x,y, by="comment")}, datalist)} #reduce funktion produziert einen output der selber wieder als Input benutz werden kann. 
...
if(exists("snakemake")){
	...
	filenames=list.files(path="data/real_izmir/datasplit/", full.names=TRUE, pattern= '0000001\\..+\\.csv') #.= beliebiges zeichen, + = whdGruppe, \csv. für dateinamen, es sollen nur csv einglesen werden
	mymergeddata <- multmerge(filenames)
	print(head(mymergeddata))
}
